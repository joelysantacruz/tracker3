...
## 1. Cancelled operations by health board (CBHB)
...
## 2. Cancelled operations Scotland (CS)
...
## Health board codes (healthboardid)
temp2 <- tempfile()
...
## Special health board codes  (specialid)
temp3 <- tempfile()
...
##country code Scotland (countryid)
temp4 <- tempfile()
...
## 3. Cancelled operations by hospital (CBHOS)
...
## filter to latest date (hospitalmapbase)
...
CBHOS %>% 
  group_by(filename) %>% 
  write_csv(.x, paste0("/data", .y$filename, ".csv"))
